...
def _valid_state(value: object) -> bool:
    ...
    valid_session = lambda item: isinstance(item, dict) and all(
        isinstance(item.get(key), str) for key in ("client_type", "device_id", "status")
    )
    valid_device = lambda item: isinstance(item, dict) and all(
        isinstance(item.get(key), str) for key in ("client_type", "first_seen_at", "last_seen_at")
    )
    valid_event = lambda item: isinstance(item, dict) and all(
        isinstance(item.get(key), str) for key in ("observed_at", "session_id", "client_type", "event", "status")
    )
    return (
