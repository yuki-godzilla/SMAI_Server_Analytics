...
`analytics_web.py` はStreamlitによる読み取り専用のWeb Operations Consoleです。SMAI本体が停止していても起動を維持し、最後のhealth snapshot、ユーザー/セッション数、実行中処理、直近ログを表示します。Windowsの通常運用入口は`run_analytics_web.bat`です。
...
- Web Consoleは信頼済みプライベートLANでのみ使用します。`run_analytics_web.bat`は明示的なLAN利用時だけ`0.0.0.0:8502`で待受し、TCP 8502をインターネットへ公開しません。ユーザーデータ、secret、token、Cookie、タスク設定の変更機能を画面へ追加しません。
- Windowsで常時監視する場合は、`scripts/register_smai_analytics_autostart_task.ps1`を実行し、対話ユーザーのログオン時に`run_analytics_web.bat`を起動します。Session 0を避け、利用者の権限・Runtimeパスで起動するためInteractiveのAtLogOnタスクを使用します。
- 自動起動登録は一度に一つの`SMAI-Server-Analytics`タスクだけを作成し、二重起動を避けます。解除は`unregister_smai_analytics_autostart_task.ps1`を使用します。
- 再起動は`restart_analytics_web.bat`を使用します。このスクリプトは`analytics_web.py`を実行しているPythonプロセスだけを停止し、SMAI本体のStreamlitプロセスを停止しません。
...
python -m py_compile analytics_web.py health.py backup.py retention.py
python -m compileall -q smai_analytics
python health.py
