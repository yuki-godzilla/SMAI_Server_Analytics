...
- ヘッダーは既存の透明背景・横長`SMAI Analytics`ロゴとマスコットを大きく使い、ロゴ・状態・最終確認・更新操作を読みやすく配置する。ブラウザタブと追加されたアプリ／ショートカットの識別には、正方形の`SMAI Analytics`アプリアイコンをStreamlitのpage iconとして使う。状態はヘッダーのpillと状態行の色で一貫して示し、マスコットは状態の視線を集める補助として使い、状態判定そのものへ依存しない。
...
- ライブ接続トポロジーとHealth 24Hは、端末画像・接続経路・推移線・マイクロ推移を読み取れる十分な縦幅を確保する。情報量を縮めるために画像や線を過度に圧縮しない。
