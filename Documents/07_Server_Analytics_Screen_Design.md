...
| Trends | 2番目 | L1〜L3の状態履歴、応答時間p95、SMAIデータ／Runtimeの空き容量、タスク実行鮮度。24時間・7日・30日を選択可能 |
...
| Tasks | 6番目 | Autostart、Watcher、Symbol Maintenance、Incident Automation、復元スモークの最終実行・次回予定・最終結果・鮮度 |
...
- Tasksでは状態だけでなく、最終実行・次回予定・最終結果・判定理由を表示する。起動契機タスクは前回起動の古さだけで異常とせず、周期が明確なIncident Automationと復元スモークだけに鮮度しきい値を適用すること。
...
- TrendsのJOB FRESHNESSは、状態変化または5分ごとに記録したScheduler観測を使い、未観測を正常色で埋めないこと。
