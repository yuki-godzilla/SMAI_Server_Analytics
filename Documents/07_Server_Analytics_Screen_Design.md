...
- ブランドは盾と大きな`SMAI Analytics`文字を重ねた正方形アプリアイコンと名称をアプリバーに置く。同じ画像をStreamlitのpage iconとしてブラウザタブと追加されたアプリ／ショートカットの識別に使う。ヘッダーではアイコンとマスコットを読み取れる大きさで並べる。状態はヘッダーのpillと状態行の色で一貫して示し、マスコットは状態の視線を集める補助として使い、状態判定そのものへ依存しない。
