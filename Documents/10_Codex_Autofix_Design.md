...
更新日: 2026-07-15
状態: v1実装済み。既定設定は`enabled=false` / `mode=dry_run`であり、専用実行IDの準備と管理者による明示有効化までは自動実行しない。
...
成功時は`auto_merged_pending_deploy`を記録し、同じ固定Gmail宛先へマージ結果を通知する。管理者は既存の`AGENTS.md`に従い、Analyticsだけを再起動し、実画面確認、通常のGit pushを別作業単位として行う。fast-forward完了後の検証だけが失敗した場合は履歴を書き換えず、`auto_merged_validation_failed`として「既にマージ済み」を明示する。自動rollback、再起動、pushは行わない。
...
1. schema v3の状態、24時間のAutofix承認、45分のrun lease、1時間のcommit hash指定マージ承認、取消、状態照会、append-only JSONL監査。
