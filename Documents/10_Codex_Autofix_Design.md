...
更新日: 2026-07-16
状態: v2実装済み。現在の設定は`enabled=true` / `mode=active` / `deployment_enabled=false`であり、critical障害での隔離修復候補作成を許可する。成功・preflight拒否・rollback成功・rollback失敗のドリル完了後にだけ、個別に第3承認されたAnalytics配備を明示的に有効化する。workerタスク未登録時は候補を実行せず、マージ・配備・再起動・pushは別の明示承認を必要とする。
