...
- TCP 8502をインターネットへ公開しません。接続できない場合のみ、Windows Firewallの**プライベート**プロファイルでTCP 8502の受信許可を確認します。Firewall変更はlive operationとして通常のコード確認とは分離します。
...
`タスク`画面は、Windows Task Schedulerから取得した最終実行時刻、次回予定、最終結果、実行パスを確認します。状態はRuntimeの `metrics/tasks/YYYY-MM-DD.jsonl` に、状態変化または5分間隔で保存され、`推移`画面の`JOB FRESHNESS`に表示されます。
