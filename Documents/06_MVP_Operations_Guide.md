...
`SMAI-Host-Monitor`は5分タスクとして`health.py`とタスク状態観測を実行します。管理者コンテキストではSYSTEM、非管理者コンテキストでは現在の対話ユーザーで登録します。どちらも画面閲覧には依存しませんが、非管理者登録はログオン中だけ実行されます。Windowsの異常停止イベント、Tailscale、メモリ、CPU、物理ディスクに異常があっても、監視タスクはWindows再起動・サービス停止・外部送信を行いません。NVIDIA GPUを検出できる場合だけ温度・ファン・消費電力をL3観測として記録し、GPU未搭載・取得不能は既存のhealth状態を悪化させません。`SMAI-Host-Monitor`は最終成功から10分超を`degraded`、20分超を`critical`として表示します。
...
healthの生記録は `Runtime/logs/health/YYYY-MM-DD.jsonl` に日別保存し、調査用に2日間保持します。画面用の5分集計は `Runtime/metrics/health/YYYY-MM-DD.jsonl` に30日間保持します。`SMAI-Runtime-Retention`は毎日03:45に既存の保持ポリシーだけを適用し、完了済みbackup・生成済みログ以外を削除しません。登録と解除は次のとおりです。
...
`タスク`画面は、Windows Task Schedulerから取得した最終実行時刻、次回予定、最終結果、実行パスの一致判定を確認します。絶対実行パスやコマンド引数は画面へ表示しません。状態はRuntimeの `metrics/tasks/YYYY-MM-DD.jsonl` に、状態変化または5分間隔で保存され、Web Consoleを開いていない時間も`SMAI-Host-Monitor`が記録します。`推移`画面の`JOB FRESHNESS`に表示されます。
...
- Task Schedulerの`0x41301`（実行中）は失敗として扱わず、実行パスが一致する場合だけ`healthy`とします。`0x41303`（登録済み・未実行）は成功と推測せず`unknown`とします。
...
月次の定着には、対話中のAnalytics運用ユーザーで次を一度だけ実行します。既定は日曜04:00の保守枠を避ける毎月1日02:00であり、同じユーザーのログオン中だけ実行します。資格情報は使わず、通常のAnalytics起動と同じPython環境を固定して起動します。

```powershell
.\scripts\register_backup_restore_smoke_task.ps1
```

登録後は、Task Schedulerの実行ユーザー、`run_backup_restore_smoke.ps1`、月次トリガー、作業対象のAnalyticsワークスペースを確認します。解除は次のとおりです。

```powershell
.\scripts\unregister_backup_restore_smoke_task.ps1
```
...
通常ログ、healthの5分集計、タスク鮮度履歴は30日、health生記録は2日、障害ログは90日を基準とします。`SMAI-Runtime-Retention`は毎日03:45にこのポリシーを適用し、削除対象は既存のretention判定に合うRuntimeログと完了済みbackupだけです。RuntimeはGit管理しません。ログにはsecret、通知topic、token、Cookie、ユーザー入力本文を記録しません。
