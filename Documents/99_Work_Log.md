...
- 自動更新の軽量化・画面安定性を改善
  - 観察: `run_every`でヘッダーと表示中タブを別周期で再描画し、ブラウザ表示の都度`health.py`を起動していた。詳細表・グラフを読んでいる間にも画面全体が更新されるように見え、同じ運用証跡の再収集も発生していた。
  - 変更: 定期更新は15秒ごとのヘッダーサマリーだけへ限定し、詳細タブは画面切替または管理者の`更新`操作まで固定した。ブラウザからの`health.py`起動を廃止し、`SMAI-Host-Monitor`が記録したhealth snapshotだけを読む構成へ変更した。snapshotの時刻が取得不能・10分超過ならfail-closedの注意表示を行う。明示的な`更新`だけはサマリー・詳細キャッシュを破棄して全詳細を取り直す。
  - 再確認: Chrome実画面でDashBoard詳細をスクロール位置`1160px`まで移動し、17秒を超えて観測した。定期更新後もスクロール位置は`1160px → 1160px`、詳細領域の高さは`2417px → 2417px`で維持され、トポロジー・Health・根拠シグナルは再描画されなかった。一方、ヘッダーの最終確認時刻は`08:49:00 → 08:49:39`へ更新された。`更新`操作では意図どおり詳細を再取得し、画面上部へ戻って新しい接続数が表示された。
