...
モバイル／タブレットのresponsive改善スプリントを完了した後の次段階は、(1) 自動起動・定期監視の実運用確認、(2) 月次復元スモークテストの定着、(3) 障害対応ドリル、(4) 狭幅を含む受け入れテストである。障害対応ドリルではcritical検知からローカル調査依頼、固定Gmailへの管理者通知、管理者承認済みのCodex修正依頼、改善レポート表示までを追跡する。管理設定はMain Applicationと同じ右上固定の「管理者SMAIナビアイコン＋管理者表示名」チップから常時開ける。管理者名称、通知先、障害／修復／復旧通知の可否はRuntimeの`incident_operations/administrator_settings.json`にのみ保存する。メニューの認証導線はGmailの通知接続設定を開き、GmailアプリパスワードはWindows Credential Managerだけに保存する。通常画面・ログ・Gitへメールアドレスやsecretを出さない。自動修復を許可しても隔離worktreeでの修復候補作成までであり、マージ、配備、再起動、pushには別の明示承認を必要とする。AnalyticsはMain Applicationのprivate moduleやユーザー状態を変更しない。SMAI本体の計算、ランキング、Forecast、ユーザーデータは変更しない。
