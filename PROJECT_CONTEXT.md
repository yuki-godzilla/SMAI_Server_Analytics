...
正規の運用画面は`analytics_web.py`（実装は`smai_analytics/ui/web_dashboard.py`）による読み取り専用のStreamlit Web Operations Consoleである。通常は`run_analytics_web.bat`で起動し、MagicDNS URLの`http://desktop-bqrpr4c:8502`からPC、タブレット、スマートフォンへ提供する。Main Applicationは同じホスト名の`http://desktop-bqrpr4c:8501`であり、Analyticsとはポートで分離する。サーバーPC内の確認だけは`http://localhost:8502`を使用する。URL設定は`config/network.json`に集約し、`0.0.0.0`は待受専用で画面・起動案内には表示しない。自動起動は`SMAI-Server-Analytics`のInteractive AtLogOnタスク、再起動は`restart_analytics_web.bat`を使用する。
