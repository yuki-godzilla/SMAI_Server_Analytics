...
class StartupWorkspaceTests(unittest.TestCase):
    ...
    def test_workspace_is_web_only_ready_gated_and_uses_rightmost_monitor(self) -> None:
        ...
        self.assertIn('"--app=$($target.page)"', pages)
        self.assertIn("Get-ExistingSmaiWindow", pages)
        self.assertIn("workspace.log", pages)
        self.assertNotIn("show_smai_service_prompt.ps1", workspace)
        ...
        self.assertIn('Name = "SMAI Main Application Web"', layout)
        self.assertIn('Name = "SMAI Analytics Web"', layout)
        self.assertIn("WorkingArea.X + $_.WorkingArea.Width", layout)
        self.assertIn("Get-HalfScreenRectangle", layout)
        self.assertEqual(layout.count('Stacking = "Normal"'), 2)
        self.assertNotIn('Stacking = "Bottom"', layout)
        self.assertNotIn("VS Code", layout)
        self.assertNotIn("Application Prompt", layout)
        self.assertNotIn("Analytics Prompt", layout)
