...
| ファイル | 対象 | 内容 |
| --- | --- | --- |
| `setup/requirements.txt` | 運用時 | ブランド画像表示用Pillow、LAN閲覧画面用Streamlit |
| `setup/requirements-dev.txt` | 開発・確認時 | pytest、ruff、black |

Tkinterは対応するWindows版CPythonに同梱されるため、pipの依存関係には含めません。

## 起動

```powershell
# ローカルTkinter運用画面
.\venv_SMAI_Analytics\Scripts\python.exe .\dashboard.py
...
.\venv_SMAI_Analytics\Scripts\python.exe -m py_compile dashboard.py analytics_web.py health.py backup.py retention.py
